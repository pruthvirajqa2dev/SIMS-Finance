...
describe("Postchecks TC 1 to 9", () => {
    ...
    it("File upload using SPC420", () => {
        ...
        cy.get(".esr_banner_environment > .title").should(
            "contain",
            "SPC420 - File Manager"
        );
        cy.screenshot(screenshotFolder + ++i);
        ...
        //View dropdown
        cy.get(".multibutton_content > a").eq(0).click();
        cy.screenshot(screenshotFolder + ++i);

        cy.get(".ui-menu-item").contains("Delete").click();
        cy.screenshot(screenshotFolder + ++i);
    ...
    it("RSS570 - Crystal Report", () => {
        ...
        //#1
        cy.get("input[aria-label=School]")
            .invoke("val")
            .should("contain", schoolId);
        //#2
        cy.get("#supplier_or_normal").type(supplierOrNominalSort, {
            force: true
        });
        //#3
        ...
        cy.task(
            ...
            cy.task("newestFileName", "./cypress/downloads/unzip*/*.PDF").then(
                (fileName) => {
    ...
    it("SIMS_TB_SCHOOL - XQuery Report - Execute", () => {
        const screenshotFolder =
            ...
            "Hour " +
            new Date().getHours() +
            "/SIMS_TB_SCHOOL - XQuery Report - Execute/";
        const username = testData.username;
        const password = testData.password;
        ...
        //Click on the menu item displayed
        cy.log("Click on the menu item displayed");
        cy.get(".ui-autocomplete-category")
            .contains("XQuery")
            .next(".ui-menu-item")
            .contains(screen)
            .click();
        ...
        //Complete form
        cy.get("#p_comp_icon").click();
        cy.screenshot(screenshotFolder + ++i);

        cy.get("tbody")
            .find('[axes="COMPANY_ID"] > div')
            .contains(schoolId)
            .parent()
            .parent()
            .find("#esr_action")
        ...
        cy.get(".TITLE_XQ")
            .invoke("text")
            .should("contain", schoolId)
            .should("contain", "Year " + period[0])
            .should("contain", "Period " + period[1]);
        cy.screenshot(screenshotFolder + ++i);
    ...
    it("SIMS_TB_SCHOOL - XQuery Report - Submit", () => {
        const screenshotFolder =
            ...
            "Hour " +
            new Date().getHours() +
            "/SIMS_TB_SCHOOL - XQuery Report - Submit/";
        const username = testData.username;
        const password = testData.password;
        const screen = testData.SIMS_TB_SCHOOL;
        ...
        cy.get("*[id^=ui-id]")
            .contains("SIMS_TB_SCHOOL - SIMS Trial Balance School")
            .should("be.visible");
        cy.screenshot(screenshotFolder + ++i);
    ...
    it("RSS310Q - Attachments", () => {
        ...
        const random = Math.floor(
            Math.random() * (Math.floor(10) - Math.ceil(1) + 1) +
                Math.ceil(1) -
                1
        );
        ...
        cy.get("@onlyFileNameWithExt").then((data) => {
            ...
            cy.get("#filename")
                .invoke("text")
                .should("contain", file + "." + ext);
        });
        cy.get("#esr_attach_button").click();
        cy.screenshot(screenshotFolder + ++i);

        cy.get("@fileNameWithoutExt").then((data) => {
            cy.get('[axes="DOCUMENT_TITLE"]')
                .contains(data)
                .should("be.visible");
            cy.get("@fileExt").then((extData) => {
                cy.get('[data-internal-ref] > [axes="DOCUMENT_TITLE"]')
                    .contains(data)
                    .eq(0)
                    .parent()
                    .siblings('[axes="FILE_EXT"]')
                    .invoke("text")
    ...
    it.only("Help Screen", () => {
        ...
        cy.url().should(
            "eq",
            "https://uat-v2.pecuniam-online.co.uk/" +
                testData.tenant +
                "/help/int/webhelp/int.htm"
        );
        cy.screenshot(screenshotFolder + ++i);
    });
});
