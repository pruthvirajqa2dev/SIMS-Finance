...
describe.skip("Scenario 30", () => {
    ...
    it("Multi line Invoice entry - SLS310 as a Finance Director for a school", () => {
        ...
        let randomUnitPrice =
            Math.random() * (unitPriceMax - unitPriceMin) + unitPriceMax;
        var resultUnitPrice =
            randomUnitPrice - Math.floor(randomUnitPrice) !== 0;
        ...
        cy.selectUsingSearchIcon(
            "custmf_code",
            String(arrCustCode[Math.floor(Math.random() * arrCustCode.length)])
        );

        cy.get("#next_button").click();
        cy.selectUsingSearchIcon(
            "prodmf_code",
            arrProd[Math.floor(Math.random() * arrProd.length)]
        );

        //    //prodmf_code //***Select customer type
        cy.selectUsingSearchIcon(
            "vatcod_code",
            arrVATCode[Math.floor(Math.random() * arrVATCode.length)]
        );
        cy.get("#qty").type(randomNum);
        cy.get("#price").type(randomUnitPrice);
